...
export function RandomQuestionFetch({match}) {
    ...
    return (
        <div>
            ...
            <div>
            {data && data.questions && data.questions.map((newQuest, key) => <li>{newQuest.question}
                {data && data.questions[key].options.map(option => <div>{option.option}</div>)}
            </li>)}
            </div>
            <nav aria-label="...">
                 <ul class="pagination pagination-md justify-content-center">
                     <li class="page-item" onClick={() => setcurrentIndex(currentIndex-1)}>Prev</li>
                      <li class="page-item" onClick={() => setcurrentIndex(currentIndex+1)}>Next</li>
                  </ul>
            </nav>
        </div>
    );
